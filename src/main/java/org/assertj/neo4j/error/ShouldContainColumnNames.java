...
public class ShouldContainColumnNames extends BasicErrorMessageFactory {
   ...
   */
  public static ErrorMessageFactory shouldContainColumnNames(ExecutionResult actual, int rowIndex, Map<String, Object> row, String... columnNames) {
    return new ShouldContainColumnNames(actual, rowIndex, row, columnNames);
  }

  private ShouldContainColumnNames(ExecutionResult actual, int rowIndex, Map<String, Object> row, String... columnNames) {
    super("\nExpecting:\n  <%s>\nwith row at index:\n  <%s>\nto contain column names\n  <%s>" +
      "\nbut row actually contains\n  <%s>", actual, rowIndex, columnNames, row, StandardComparisonStrategy.instance());
  }
}
