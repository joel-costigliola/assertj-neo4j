...
public class ShouldContainEntries extends BasicErrorMessageFactory {
   ...
   */
  public static ErrorMessageFactory shouldContainEntries(ExecutionResult actual, int rowIndex, Map<String, Object> row,
      MapEntry[] entries, Collection<MapEntry> notFound) {
    return new ShouldContainEntries(actual, rowIndex, row, entries, notFound);
  }

  private ShouldContainEntries(ExecutionResult actual, int rowIndex, Map<String, Object> row, MapEntry[] entries,
      Collection<MapEntry> notFound) {
    super("\nExpecting:\n  <%s>\nwith row at index:\n  <%s>\nto contain\n  <%s>"
        + "\nwhile row actually contains\n  <%s>" + "\ncould not find\n  <%s>", actual, rowIndex, entries, row,
        notFound, StandardComparisonStrategy.instance());
  }
}
