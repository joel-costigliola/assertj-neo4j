...
/**
 * Checks <code>{@link org.assertj.neo4j.api.PropertyContainerAssert#hasNotProperty(String,Object)}</code> behavior.
 * 
...
public class PropertyContainerAssert_hasNotProperty_key_value_Test {
  ...
  public void should_pass_hasNotProperty_when_property_container_contains_key_but_NOT_value() {
    ...
    assertThat(propertyContainer).hasNotProperty("name", "Peter Neubauer");
  }

  @Test
  public void should_fail_hasNotProperty_when_passed_property_key_is_null() {
    expectedException.expect(IllegalArgumentException.class);
    expectedException.expectMessage("The key to look for should not be null");

    given_property_container_with_key_and_value("name", "Emil Eifrem");

    assertThat(propertyContainer).hasNotProperty(null, "Emil Eifrem");
  }

  @Test
  public void should_fail_hasNotProperty_when_passed_property_value_is_null() {
    expectedException.expect(IllegalArgumentException.class);
    expectedException.expectMessage("The value to look for should not be null");

    given_property_container_with_key_and_value("name", "Emil Eifrem");

    assertThat(propertyContainer).hasNotProperty("name", null);
  }
  ...
  public void should_fail_hasNotProperty_when_property_container_contains_key_AND_value() {
    ...
    assertThat(propertyContainer).hasNotProperty("name", "Emil Eifrem");
  }
