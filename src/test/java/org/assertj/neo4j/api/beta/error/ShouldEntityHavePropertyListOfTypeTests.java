...
class ShouldEntityHavePropertyListOfTypeTests {
    ...
    class CreateTests {
        ...
        void should_generate_error_message() {
            // GIVEN
            final DbNode node = Entities.node().id(3).property("key", Arrays.asList(3.11, 3.12, 3.13)).build();
    ...
    class ElementsTests {
        ...
        void should_generate_an_aggregate_error_message() {
            // GIVEN
            final DbNode node1 = Entities.node().id(1).property("key", emptyList()).build();
            final DbNode node2 = Entities.node().id(2).property("key", singletonList("v-2.1.1")).build();
            final DbNode node3 = Entities.node().id(3).property("key", asList(3.11, 3.12)).build();
            final DbNode node4 = Entities.node().id(4).property("key", asList(6, 5, 4)).build();
